...
function classifyUpdateLabel(haystack: string, fallback: string) {
	...
	return labels.find(([needle]) => normalized.includes(needle))?.[1] ?? fallback;
}
...
export function parsePdfLinks(html: string, sourcePageUrl: string) {
	...
	for (const unit of candidateRows) {
		...
		const pdfHrefs = hrefs.filter((href) => href.toLowerCase().includes(".pdf"));
		...
		for (const href of pdfHrefs) {
			...
			const anchor = anchors.find((candidate) => $(candidate).attr("href") === href);
			const anchorText = anchor ? cleanText($(anchor).text()) : "";
			const fileTitle =
				safeDecode(pdfUrl).split("/").pop()?.replace(/\.pdf$/i, "") ||
				"Concours PDF";
			...
			links.set(pdfUrl, {
				...
				isImportant:
					target.isNotice || !target.isFollowUpDocument || classifyImportance(linkHaystack),
			});
		}
	}
